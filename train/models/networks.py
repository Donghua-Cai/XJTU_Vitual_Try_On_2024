...
class VGGLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if (x.shape[-1] < 16) or (x.shape[-2]< 16):
            ...
            if x.shape[-2] < 16 :
                xpading = 16-x.shape[-2]
            else:
                xpading =0
            if x.shape[-1] < 16:
                ypading = 16-x.shape[-1]
            else:
                ypading = 0
            # padd = nn.ZeroPad2d(padding=(0, 0, ypading, xpading))
            # x = padd(x)
            # y = padd(y)
            x = torch.nn.functional.pad(x,[ypading//2,ypading//2,xpading//2,xpading//2], "constant", 0)
            y = torch.nn.functional.pad(y,[ypading//2,ypading//2,xpading//2,xpading//2] ,"constant", 0)
